...
import { fetchAllAdminReservasi, updateReservasiStatus as updateStatusReservasi } from '../../api/reservasi.js'
...
export default function Reservasi() {
  ...
  const load = async () => {
    ...
    try {
      ...
      // API response -> every page -> normalizeReservasi (single price/space/member
      // derivation shared with member pages, dashboard and reports) -> state.
      const rows = await fetchAllAdminReservasi(params)
  ...
  const columns = [
    {
      ...
      render: (r) => (
        <div>
          <div className="font-medium text-ink">{r.nama_member || 'Member'}</div>
  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto">
      <div>
        ...
        <p className="text-stone">Pantau dan kelola seluruh reservasi member.</p>
      </div>
